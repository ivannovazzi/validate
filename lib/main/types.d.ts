...
export type Tester<V extends Source, S extends Source = V> = [V] extends [Primitive] ? Validator<V, S> : V extends Nested ? NestedTester<V, S> : never;
export type NestedResults<T extends Nested> = {
    [K in keyof T]: Results<T[K]>;
};
export type Results<T extends Source> = T extends Primitive ? Result : T extends Nested ? NestedResults<T> : never;
export type TestingFunc<T = unknown, S = unknown> = ({ value, source }: {
    value: T;
    source: S;
}) => boolean;
